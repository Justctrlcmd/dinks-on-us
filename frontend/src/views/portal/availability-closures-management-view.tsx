...
function ActivityCard({ activity }: { activity: AvailabilityActivity }) {
  ...
  return <article className="flex min-h-16 gap-2.5 rounded-lg border border-border bg-background p-2.5"><span className={reopened ? "grid size-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary" : "grid size-8 shrink-0 place-items-center rounded-lg bg-destructive/10 text-destructive"}><Icon aria-hidden="true" className="size-3.5" /></span><div className="min-w-0 flex-1"><h3 className="font-heading text-sm font-bold leading-5">{activityTitle(activity)}</h3><p className="mt-0.5 text-xs text-muted-foreground"><span className="font-medium text-foreground">Internal reason:</span> {activity.details.reason}</p><p className="mt-1 text-xs text-muted-foreground">{reopened ? "Reopened" : "Set"} {formatDateTime(activity.created_at)} by {activity.actor_name}</p></div></article>;
}
...
export function AvailabilityClosuresManagementView() {
  ...
  async function reopen() {
    if (!reopening) return;
    try {
      await reopenMutation.mutateAsync(reopening);
      setReopening(null);
  ...
  return (
    <div className="flex flex-col gap-4 xl:min-h-[calc(100svh-4rem)]">
      ...
      <Dialog open={Boolean(reopening)} onOpenChange={(open) => !open && !reopenMutation.isPending && setReopening(null)}><DialogContent><DialogHeader><DialogTitle>Reopen this closure?</DialogTitle><DialogDescription>{reopening?.type === "entire_operation" ? `All courts will become available again for ${reopening ? formatDateOnly(reopening.date) : "this date"}, unless another court-time closure still applies.` : `The selected ${reopening?.court?.name ?? "court"} time ranges will become available again.`}</DialogDescription></DialogHeader><DialogFooter><DialogClose render={<Button variant="outline" disabled={reopenMutation.isPending} />}>Cancel</DialogClose><Button disabled={reopenMutation.isPending} onClick={() => void reopen()}>{reopenMutation.isPending ? "Reopening…" : "Reopen closure"}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
