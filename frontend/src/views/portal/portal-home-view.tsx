...
> = {
  ...
  ONGOING: {
    label: "Ongoing",
    className: "border-energy bg-energy/15 text-foreground",
    icon: FiPlay,
  },
  COMPLETED: {
    label: "Completed",
  ...
  PAST: {
    label: "Past",
    className: "border-border/70 bg-muted/45 text-muted-foreground opacity-65",
    icon: FiClock,
  },
};

const viewableStatuses: DashboardSlotStatus[] = [
  "PENDING",
  "VERIFIED",
  "ONGOING",
  "COMPLETED",
      ...
      <CardContent>
        <div className="grid w-full min-w-0 grid-cols-7 gap-1 sm:gap-2">
          {days.map((day) => {
            ...
            const suffix = day.is_closed
              ? "Closed"
              : day.is_past
                ? "Past"
                : `${day.available_slots} of ${day.total_slots} available`;
            ...
            return (
              ...
              >
                ...
                <span className="block text-[0.5rem] font-bold uppercase tracking-[.05em] text-muted-foreground sm:text-[0.65rem] sm:tracking-[.12em]">
                  {weekday.format(parsed)}
                </span>
                <span className="mt-0.5 block font-heading text-lg font-extrabold sm:mt-1 sm:text-2xl">
                  {parsed.getDate()}
                </span>
                <span className="mt-0.5 block text-[0.5rem] font-semibold leading-tight sm:hidden">
                  {day.is_closed
                    ? "Closed"
                    : day.is_past
                      ? "Past"
...
function TimeSlotCard({
  ...
  const viewable =
    viewableStatuses.includes(slot.status) && slot.reservation_id !== null;
  const content = (
    <>
      <span
        className="grid size-8 shrink-0 place-items-center rounded-lg bg-background/60"
        aria-hidden
      >
      ...
      <span className="min-w-0 flex-1 text-left">
        {showCourtName ? (
          <span className="mb-0.5 block text-[0.65rem] font-bold uppercase tracking-[.1em]">
            {courtName}
          </span>
        ) : null}
        <span className="hidden font-heading text-sm font-bold text-foreground sm:block">
          {formatHourRange(slot.start_hour, slot.end_hour)}
        </span>
        <span className="mt-0.5 block text-xs font-semibold">
          {presentation.label}
          {slot.reservation_reference ? ` · ${slot.reservation_reference}` : ""}
        </span>
      </span>
  ...
  const className = cn(
    ...
    viewable &&
      "cursor-pointer transition-colors hover:brightness-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring dark:hover:brightness-110",
  );
  ...
  if (viewable) {
    return (
      <button
        type="button"
        className={className}
        aria-label={`View ${presentation.label.toLowerCase()} reservation ${slot.reservation_reference} for ${courtName}, ${formatHourRange(slot.start_hour, slot.end_hour)}`}
        onClick={() => onView(slot.reservation_id as number)}
      >
        {content}
      </button>
    );
  ...
  return (
    <button
      ...
      aria-label={`${courtName}, ${formatHourRange(slot.start_hour, slot.end_hour)}, ${presentation.label}`}
    >
      {content}
    </button>
  );
...
function CourtSchedule({
  ...
  courts: Array<{ id: number; name: string; slots: DashboardSlot[] }>;
  onView: (id: number) => void;
}) {
    ...
    <div aria-label="Court schedule">
      <div className="hidden overflow-x-auto pb-1 md:block">
        <div className="grid min-w-full gap-3 md:grid-flow-col md:grid-cols-none md:auto-cols-[minmax(14rem,1fr)]">
          {courts.map((court) => (
            <section
              key={court.id}
              aria-labelledby={`dashboard-court-${court.id}`}
              className="min-w-0"
            >
              <h3
                id={`dashboard-court-${court.id}`}
                className="flex min-h-11 items-center justify-center rounded-xl border border-primary/25 bg-primary/10 px-3 text-center font-heading font-bold text-primary"
              >
                {court.name}
              </h3>
      ...
      <div className="grid gap-3 md:hidden">
        {timeRows.map((timeSlot) => (
          <article
            key={timeSlot.start_hour}
            className="rounded-xl border bg-background p-3"
          >
            <h3 className="text-sm font-bold">
              {formatHourRange(timeSlot.start_hour, timeSlot.end_hour)}
            </h3>
            <div className="mt-3 grid gap-2">
              {courts.map((court) => {
                const slot = court.slots.find(
                  (candidate) => candidate.start_hour === timeSlot.start_hour,
                );
                return slot ? (
                  <TimeSlotCard
                    key={`${court.id}-${timeSlot.start_hour}`}
                    slot={slot}
                    courtName={court.name}
                    showCourtName
                    onView={onView}
...
export function PortalHomeView() {
  ...
  return (
    <div className="grid gap-5">
      ...
      <section
        aria-label="Weekly dashboard KPIs"
      ...
      >
        ...
        <PortalMetricCard
          label="Revenue"
          value={data ? currency.format(data.kpis.revenue) : undefined}
      ...
      {query.isPending ? (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            Loading dashboard availability…
          </CardContent>
        </Card>
      ) : query.isError || !data ? (
        <Card>
          <ErrorState
            ...
            description="Try loading the weekly dashboard again."
        ...
        <>
          ...
          <Card aria-labelledby="dashboard-time-slots-title">
            <CardHeader className="border-b">
              <CardTitle
                id="dashboard-time-slots-title"
                className="truncate text-base font-bold leading-tight sm:text-lg sm:whitespace-normal"
              >
                Time slots for{" "}
                {longDate.format(parseDateOnly(data.selected_date.date))}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {data.selected_date.courts.length === 0 ? (
                <EmptyState
                  title="Court availability is not configured"
                  description="Active courts and operating hours will appear here once configured."
                />
              ) : (
                <div className="grid gap-4">
                  {data.selected_date.is_closed ? (
                    <p
                      role="status"
                      className="rounded-xl border border-destructive/25 bg-destructive/10 px-4 py-3 text-sm font-medium"
                    >
                      The entire operation is closed for this date.
                    </p>
                  ) : null}
                  <CourtSchedule
                    courts={data.selected_date.courts}
                    onView={setSelectedReservationId}
                  />
                </div>
              )}
            </CardContent>
