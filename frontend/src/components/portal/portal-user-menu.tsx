...
import { canAccessPortalModule, canManageOwnProfile } from "@/config/navigation";
...
export function PortalUserMenu({ user, collapsed = false }: { user: User; collapsed?: boolean }) {
  ...
  return <DropdownMenu><DropdownMenuTrigger render={trigger} /><DropdownMenuContent side="top" align="start" className="min-w-56">{canManageOwnProfile(user) ? <DropdownMenuItem render={<Link href="/portal/profile" />}><FiUser aria-hidden="true" />Profile</DropdownMenuItem> : null}<DropdownMenuItem disabled={!mounted} onClick={() => setTheme(darkMode ? "light" : "dark")}>{darkMode ? <FiSun aria-hidden="true" /> : <FiMoon aria-hidden="true" />}{darkMode ? "Light Mode" : "Dark Mode"}</DropdownMenuItem>{canAccessPortalModule(user, "RESERVATION") ? <PushNotificationSettings /> : null}<DropdownMenuSeparator /><DropdownMenuItem variant="destructive" disabled={logout.isPending || isMutationRateLimited(logout)} onClick={async () => { await logout.mutateAsync(); router.replace("/login"); }}><FiLogOut aria-hidden="true" /><span>{mutationButtonLabel("Logging out…", "Logout", logout)}</span></DropdownMenuItem></DropdownMenuContent></DropdownMenu>;
}
