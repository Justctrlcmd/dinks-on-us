...
function WeekSelector({ selectedDate, weekStart, today, maxBookingDate, closedDates, lockedDate, onSelectDate, onChangeWeek }: { selectedDate: string; weekStart: string; today: string; maxBookingDate?: string; closedDates: ReadonlySet<string>; lockedDate?: string; onSelectDate: (date: string) => void; onChangeWeek: (amount: number) => void }) {
    ...
    <section className="rounded-2xl border border-border bg-card p-3 sm:p-6" aria-labelledby="weekly-availability-title">
      ...
      <div className="mt-3 grid grid-cols-7 gap-1.5 sm:mt-5 sm:gap-2">
        {dates.map((date) => {
          ...
          return <button key={date} type="button" disabled={isPast || isAfterBookingWindow || isClosed || locked} aria-label={`${weekday.format(parsed)} ${parsed.getDate()}${isAfterBookingWindow ? ", outside online booking window" : isClosed ? ", Closed" : locked ? ", unavailable while another date is selected" : selected ? ", selected" : ""}`} aria-pressed={selected} onClick={() => onSelectDate(date)} className={cn("min-w-0 rounded-lg border px-1 py-2 text-center transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:cursor-not-allowed sm:min-h-28 sm:rounded-xl sm:px-3 sm:py-3", selected && "border-primary bg-primary text-primary-foreground shadow-sm", !selected && !isPast && !isAfterBookingWindow && !isClosed && !locked && "border-border bg-background hover:border-primary/55 hover:bg-muted", (isPast || isAfterBookingWindow || locked) && "border-border/60 bg-muted/45 text-muted-foreground opacity-55", isClosed && "border-destructive/35 bg-destructive/10 text-destructive")}>
            <span className="block text-[0.58rem] font-bold uppercase tracking-[.08em] opacity-75 sm:text-xs sm:tracking-[.14em]">{weekday.format(parsed)}</span><span className="mt-0.5 block font-heading text-lg font-extrabold sm:mt-1 sm:text-2xl">{parsed.getDate()}</span><span className="mt-1 hidden text-xs font-bold sm:block">{isAfterBookingWindow ? "Unavailable" : isClosed ? "Closed" : isPast ? "Past" : locked ? "Other date selected" : selected ? "Selected" : "View slots"}</span>
          </button>;
        })}
      </div>
    </section>
