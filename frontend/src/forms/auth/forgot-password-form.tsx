...
export function ForgotPasswordForm() {
  const mutation = useForgotPassword(); const [message, setMessage] = useState<string>(); const [sent, setSent] = useState(false);
  const form = useForm<ForgotPasswordValues>({ resolver: zodResolver(forgotPasswordSchema), defaultValues: { email: "" } });
  const submit = form.handleSubmit(async ({ email }) => { setMessage(undefined); try { await mutation.mutateAsync(email); setSent(true); } catch (error) { setMessage(applyApiErrors(error, form.setError)); } });
  return <form onSubmit={submit} className="grid gap-4" noValidate>
    {(message || sent) && <Alert variant={message ? "destructive" : "default"}><AlertDescription>{message ?? "If an account exists for that email, a reset link has been sent."}</AlertDescription></Alert>}
    ...
    <Button disabled={mutation.isPending} type="submit">{mutation.isPending ? "Sending..." : "Send reset link"}</Button>
