...
import {
  ...
  type ReservationSlotSelection as SlotSelection,
...
function Field({ label, htmlFor, required, children }: { label: string; htmlFor?: string; required?: boolean; children: React.ReactNode }) {
  return <div className="grid gap-2"><Label htmlFor={htmlFor}>{label}{required ? <span aria-hidden="true"> *</span> : null}</Label>{children}</div>;
}
...
export function ReservationDetailDialog({ reservation, reservationId, open, onOpenChange, source = "reservations" }: { reservation: ManagementReservation | null; reservationId?: number | null; open: boolean; onOpenChange: (open: boolean) => void; source?: "reservations" | "history" | "dashboard" }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-3xl">
    ...
    {detail.isPending && !item ? <p className="py-8 text-center text-sm text-muted-foreground">Loading reservation details…</p> : item ? <div className="grid gap-5">
      <section className="rounded-xl border p-4"><h3 className="font-heading font-bold">Customer information</h3><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="text-muted-foreground">Name</dt><dd className="font-medium">{item.customer.name}</dd></div><div><dt className="text-muted-foreground">Email</dt><dd className="break-all font-medium">{item.customer.email}</dd></div><div><dt className="text-muted-foreground">Contact</dt><dd className="font-medium">{item.customer.contact_number}</dd></div><div><dt className="text-muted-foreground">Source</dt><dd className="font-medium capitalize">{item.source.toLowerCase().replaceAll("_", "-")}</dd></div></dl></section>
...
export function RejectReservationDialog({ reservation, open, onOpenChange }: { reservation: ManagementReservation | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [concernSelection, setConcernSelection] = useState<{ reservationId: number | null; value: string }>({ reservationId: null, value: concernOptions[0][0] });
  const concern = concernSelection.reservationId === reservation?.id ? concernSelection.value : concernOptions[0][0];
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!reservation) return; const data = new FormData(event.currentTarget); mutation.mutate({ id: reservation.id, concern, reason: String(data.get("reason")) }, { onSuccess: () => onOpenChange(false) }); }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-lg"><form key={`${reservation?.id ?? "none"}-${open}`} onSubmit={submit} className="grid gap-5"><DialogHeader><DialogTitle>Reject {reservation?.reference_number}?</DialogTitle><DialogDescription>This finalizes the reservation and immediately releases its court times.</DialogDescription></DialogHeader>
    <SelectWithLabel id="reservation-concern" label="Concern" required value={concern} options={concernOptions.map(([value, label]) => ({ value, label }))} onValueChange={(value) => { if (value) setConcernSelection({ reservationId: reservation?.id ?? null, value }); }} />
    <Field label="Reason" htmlFor="reservation-rejection-reason" required><Textarea id="reservation-rejection-reason" name="reason" required maxLength={1500} rows={5} placeholder="Explain why this reservation cannot be accepted." /></Field>
...
function RescheduleForm({ reservation, onDone }: { reservation: ManagementReservation; onDone: () => void }) {
  ...
  const [ranges, setRanges] = useState<SlotSelection[]>(() => Array.from({ length: requiredCount }, () => ({ courtId: "", slot: "" })));
  const mutation = useRescheduleReservation();
  const slotCount = expandRanges(date, ranges).length;
  function submit(event: FormEvent) { event.preventDefault(); if (slotCount !== requiredCount) return; mutation.mutate({ id: reservation.id, slots: expandRanges(date, ranges) }, { onSuccess: onDone }); }
  return <form onSubmit={submit} className="grid gap-5"><DialogHeader><DialogTitle>Reschedule {reservation.reference_number}?</DialogTitle><DialogDescription>Choose exactly {requiredCount} one-hour slots. Higher rates create a balance; lower rates create refundable credit.</DialogDescription></DialogHeader>
    <ScheduleFields date={date} setDate={setDate} ranges={ranges} setRanges={setRanges} allowedCurrentSlots={reservation.slots ?? []} />
...
export function AddOnsDialog({ reservation, open, onOpenChange }: { reservation: ManagementReservation | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [ranges, setRanges] = useState<SlotSelection[]>([{ courtId: "", slot: "" }]);
  const [paymentChannel, setPaymentChannel] = useState<ReservationPaymentChannel | null>(null);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-3xl"><form key={`${reservation?.id ?? "none"}-${open}`} onSubmit={submit} className="grid gap-4"><DialogHeader><DialogTitle>Add these items to {reservation?.reference_number}?</DialogTitle><DialogDescription>Add available court time, players, or rental equipment to this ongoing reservation.</DialogDescription></DialogHeader>
    ...
    {hasInput ? <>
      ...
      <section className="grid gap-4 rounded-xl border p-4">
        ...
        <SelectWithLabel id="reservation-addon-payment-channel" label="Payment method" required value={paymentChannel} options={[{ value: "CASH", label: "Cash" }, { value: "EWALLET", label: "E-wallet" }, { value: "BANK", label: "Bank" }]} placeholder="Select payment method" onValueChange={(value) => setPaymentChannel(value as ReservationPaymentChannel | null)} />
        <Field label="Transaction reference (optional)"><Input name="payment_reference_number" /></Field>
        <Field label="Payment receipt (optional)"><Input name="payment_proof" type="file" accept="image/jpeg,image/png,image/webp" /></Field>
      </section>
    </> : null}
...
export function CompleteReservationDialog({ reservation, open, onOpenChange }: { reservation: ManagementReservation | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [paymentChannelSelection, setPaymentChannelSelection] = useState<{ reservationId: number | null; value: string | null }>({ reservationId: null, value: null });
  const paymentChannel = paymentChannelSelection.reservationId === reservation?.id ? paymentChannelSelection.value : null;
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!reservation) return; const source = new FormData(event.currentTarget); const input = new FormData(); if ((reservation.amounts.outstanding ?? 0) > 0 && !paymentChannel) return; if (paymentChannel) input.set("payment_channel", paymentChannel); const reference = source.get("payment_reference_number"); if (reference) input.set("payment_reference_number", String(reference)); const proof = source.get("payment_proof"); if (proof instanceof File && proof.size > 0) input.set("payment_proof", proof); mutation.mutate({ id: reservation.id, input }, { onSuccess: () => onOpenChange(false) }); }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-lg"><form key={`${reservation?.id ?? "none"}-${open}`} onSubmit={submit} className="grid gap-5"><DialogHeader><DialogTitle>Complete {reservation?.reference_number}?</DialogTitle><DialogDescription>This finalizes service and records the final amount as business data.</DialogDescription></DialogHeader>
    ...
    {(reservation?.amounts.outstanding ?? 0) > 0 ? <><SelectWithLabel id="reservation-payment-channel" label="How was the additional amount collected?" required value={paymentChannel} options={[{ value: "CASH", label: "Cash" }, { value: "EWALLET", label: "E-wallet" }, { value: "BANK", label: "Bank" }]} placeholder="Select payment channel" onValueChange={(value) => setPaymentChannelSelection({ reservationId: reservation?.id ?? null, value })} /><Field label="Transaction reference (optional)"><Input name="payment_reference_number" /></Field><Field label="Payment proof (optional)"><Input name="payment_proof" type="file" accept="image/jpeg,image/png,image/webp" /></Field></> : null}
...
export function CancelReservationDialog({ reservation, open, onOpenChange }: { reservation: ManagementReservation | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!reservation) return; const data = new FormData(event.currentTarget); mutation.mutate({ id: reservation.id, input: { reason: String(data.get("reason")), refund_type: refundType, ...(refundType === "CUSTOM" ? { refund_amount: Number(data.get("refund_amount")) } : {}) } }, { onSuccess: () => onOpenChange(false) }); }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-lg"><form key={`${reservation?.id ?? "none"}-${open}`} onSubmit={submit} className="grid gap-5"><DialogHeader><DialogTitle>Cancel {reservation?.reference_number}?</DialogTitle><DialogDescription>Use this only for an approved force majeure cancellation. Court times will be released.</DialogDescription></DialogHeader><Field label="Cancellation reason" htmlFor="reservation-cancellation-reason" required><Textarea id="reservation-cancellation-reason" name="reason" required rows={5} /></Field><SelectWithLabel id="reservation-refund" label="Refund" required value={refundType} options={[{ value: "FULL", label: `Full refund · ${currency.format(reservation?.amounts.paid ?? 0)}` }, { value: "CUSTOM", label: "Custom refund" }]} onValueChange={(value) => { if (value === "FULL" || value === "CUSTOM") setRefundType(value); }} />{refundType === "CUSTOM" ? <Field label="Custom refund amount" htmlFor="reservation-custom-refund" required><Input id="reservation-custom-refund" name="refund_amount" type="number" min="0" max={reservation?.amounts.paid} step="0.01" required /></Field> : null}<DialogFooter><DialogClose render={<Button type="button" variant="outline">Keep reservation</Button>} /><Button type="submit" variant="destructive" disabled={mutation.isPending}>{mutation.isPending ? "Cancelling…" : "Yes, cancel and record refund"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
